...
private let  UpdateTimeEntryPathBase = "/time_entries/%@"

internal class UpdateTimeEntryRequest: NetworkRequest<TimeEntryDetailsResponse, TimeEntryDetailsResult> {
    private let id: Int
    private let details: TimeEntrySendDetails
    internal init(id: Int, details: TimeEntrySendDetails) {
        self.id = id
        self.details = details
    }
    
    override func performRequest() {
        let path = String(format: UpdateTimeEntryPathBase, NSNumber(value: id))
